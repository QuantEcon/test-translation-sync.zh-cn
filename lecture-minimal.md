...
模型的**校准**（calibration）是指选择一组参数值，使模型的预测与观测数据相吻合。校准不是估计：它通过匹配少数几个目标矩来确定参数，而不是通过最大化似然函数（likelihood）。诸如 McCall 模型这样的搜寻模型通常就是这样校准的。统计学背景可参阅 [维基百科的校准条目](https://en.wikipedia.org/wiki/Calibration_(statistics))。
