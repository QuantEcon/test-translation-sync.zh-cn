...
translation:
  ...
  headings:
    ...
    The Prisoner's Dilemma::Real-World Applications: 现实世界应用
...
博弈论提供了一个分析多个决策者进行策略互动情形的框架。每个参与者的最优选择都取决于其他人的选择，从而产生复杂的相互依赖关系。

在经济市场中，进行价格或数量竞争的企业会参与策略博弈。理解这些互动有助于预测市场结果并设计更好的机制。

## 囚徒困境

经典的囚徒困境说明了为什么即使合作对所有人都有利，理性个体也可能不会选择合作。两名嫌疑人被分别审讯，必须决定是坦白还是保持沉默。
...
每个参与者都有一个占优策略是背叛，从而导致对双方都不理想的结果。
...
理解这一博弈有助于解释市场失灵以及监管或合作机制存在的必要性。

## 纳什均衡

纳什均衡是这样一种策略组合：在该组合下，任何一个参与者都无法通过单方面改变策略来提高自己的收益。它代表了一种稳定的结果，即在给定他人策略的情况下，每个参与者的策略都是最优的。

用数学语言表述，对于参与者 $i = 1, \ldots, n$，其策略集为 $S_i$，收益函数为 $u_i$，如果策略组合 $(s_1^*, \ldots, s_n^*)$ 满足以下条件，则它是一个纳什均衡：
...
当不存在纯策略纳什均衡时，参与者可能会在各策略之间进行随机化。这些混合策略均衡需要满足无差异条件，使参与者愿意进行随机化。
...
在序贯博弈中，参与者按特定顺序行动，并能观察到之前的行动。这类博弈通过逆向归纳法进行分析，即从博弈树的末端向开端进行推理。

其中的关键洞见是子博弈完美性：在每个决策节点上，参与者都会在给定未来行动的情况下做出最优选择。这消除了理性参与者不会执行的不可信威胁。

### 斯塔克尔伯格竞争

一个经典的应用是斯塔克尔伯格竞争，其中一家企业（领导者）首先选择数量，另一家企业（跟随者）在观察到这一选择后再做决策。

领导者会预见到跟随者的反应函数，并做出最优选择：
...
其中的关键机制是：如果有人偏离合作路径，就会威胁回归到非合作行为。这需要满足：
...
一种常见的执行机制是冷酷策略：合作直到有人背叛，此后则永远背叛。当满足以下条件时，该策略可以维持合作：

$$
\frac{1}{1-\delta} \cdot \pi_{\text{coop}} \geq \pi_{\text{deviate}} + \frac{\delta}{1-\delta} \cdot \pi_{\text{punish}}
$$

左边是永久合作所带来的收益，而右边是偏离行为所带来的收益加上此后永久惩罚所带来的收益。

## 练习

1. **计算均衡**：为以下博弈找出所有纳什均衡（纯策略和混合策略）：
...
3. **序贯博弈**：求解一个三阶段进入博弈中的子博弈完美均衡，在该博弈中，企业依次决定是否进入某一市场。

4. **混合策略**：验证你在练习 1 中计算出的混合策略均衡，确实使两位参与者在其纯策略之间保持无差异。