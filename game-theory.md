...
博弈论为分析多个决策者进行策略互动的情况提供了一个框架。每个参与者的最优选择取决于其他人的选择,从而产生复杂的相互依赖关系。

在经济市场中,企业在价格或数量上竞争时会进行策略博弈。理解这些互动有助于预测市场结果并设计更好的机制。
...
纳什均衡是一种策略组合,在该组合中没有参与者可以通过单方面改变策略来改善自己的收益。它代表一个稳定的结果,其中每个参与者的策略在给定其他人策略的情况下是最优的。
...
当不存在纯策略纳什均衡时,参与者可能会对策略进行随机化。这些混合策略均衡需要无差异条件,使参与者愿意进行随机化。
...
在序贯博弈中,参与者按特定顺序行动并观察先前的行动。这些博弈使用逆向归纳法分析,从博弈树的末端向开始处推进。

关键洞察是子博弈完美性:在每个决策节点,参与者在给定未来博弈的情况下做出最优选择。这消除了理性参与者不会执行的不可信威胁。

### 斯塔克尔伯格竞争

一个经典应用是斯塔克尔伯格竞争,其中一家企业(领导者)首先选择产量,另一家企业(追随者)在观察这一选择后再做决定。
...
当博弈重复进行时,通过声誉和惩罚机制会出现新的均衡。民间定理表明,当参与者足够有耐心时,许多结果可以作为均衡得以维持。

关键机制是如果有人偏离合作路径就威胁回归到非合作博弈。这需要:
...
2. **重复博弈**:使用冷酷触发策略,计算在无限重复囚徒困境中维持合作所需的最小贴现因子。

3. **序贯博弈**:求解三阶段进入博弈中的子博弈完美均衡,其中企业按顺序决定是否进入市场。

4. **混合策略**:验证你为练习1计算的混合策略均衡使两个参与者对其纯策略都无差异。