...
博弈论为分析多个决策者策略性互动的情况提供了一个框架。每个参与者的最优选择取决于其他人的选择,从而产生复杂的相互依赖关系。

在经济市场中,企业在价格或数量上的竞争构成策略性博弈。理解这些互动有助于预测市场结果并设计更好的机制。

## 囚徒困境

经典的囚徒困境说明了为什么理性个体可能不会合作,即使合作对每个人都有利。两名嫌疑人被分别审讯,必须决定是坦白还是保持沉默。
...
纳什均衡是一种策略组合,其中没有参与者可以通过单方面改变策略来改善其收益。它代表了一种稳定结果,其中每个参与者的策略在给定其他人策略的情况下是最优的。
...
当不存在纯策略纳什均衡时,参与者可能在策略上随机化。这些混合策略均衡需要无差异条件,即参与者愿意随机化。
...
在序贯博弈中,参与者按特定顺序行动并观察先前的行动。这些博弈使用逆向归纳法分析,从博弈树的末端向开端推进。

关键洞见是子博弈完美:在每个决策节点,参与者在给定未来博弈的情况下做出最优选择。这消除了理性参与者不会执行的不可信威胁。

### 斯塔克尔伯格竞争

一个经典应用是斯塔克尔伯格竞争,其中一个企业(领导者)首先选择产量,另一个企业(跟随者)在观察到这个选择后再做决定。
...
4. **混合策略**:验证你为练习1计算的混合策略均衡使两个参与者对其纯策略无差异。