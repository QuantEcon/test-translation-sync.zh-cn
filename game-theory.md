...
translation:
  title: 博弈论基础
  headings:
    Introduction to Strategic Thinking: 策略性思维导论
...
博弈论提供了一个用于分析多个决策者进行策略性互动情形的框架。每个参与者的最优选择都取决于其他参与者的选择，从而产生复杂的相互依赖关系。

在经济市场中，就价格或数量进行竞争的企业之间会进行策略性博弈。理解这些互动有助于预测市场结果并设计更好的机制。

## 囚徒困境

经典的囚徒困境说明了为什么理性个体即使在合作能使所有人受益的情况下，也可能无法合作。两名嫌疑人被分别审讯，必须决定是坦白还是保持沉默。
...
每个参与者都有一个占优策略是选择背叛，这导致对双方来说都是次优的结果。
...
纳什均衡是一种策略组合，在该组合下，任何参与者都无法通过单方面改变策略来提高自身收益。它代表一种稳定的结果，即在其他参与者策略给定的情况下，每个参与者的策略都是最优的。

在数学上，对于参与者 $i = 1, \ldots, n$，其策略集为 $S_i$，收益函数为 $u_i$，如果满足以下条件，则策略组合 $(s_1^*, \ldots, s_n^*)$ 是一个纳什均衡：
...
在序贯博弈中，参与者按特定顺序行动，并能观察到先前的行动。这类博弈通过反向归纳法进行分析，即从博弈树的末端向开端逆向求解。

其中的关键洞见是子博弈完美性：在每个决策节点上，参与者在给定未来博弈进程的情况下作出最优选择。这消除了理性参与者不会执行的不可信威胁。

### 斯塔克尔伯格竞争

一个经典的应用是斯塔克尔伯格竞争，其中一家企业（领导者）先选择产量，另一家企业（跟随者）在观察到这一选择后再做决策。

领导者会预见到跟随者的反应函数，并据此作出最优选择：
...
当博弈被反复进行时，通过声誉与惩罚机制会产生新的均衡。无名氏定理表明，当参与者足够有耐心时，许多结果都可以作为均衡被维持下去。

其中的关键机制是：如果任何人偏离合作路径，就会受到回归非合作博弈的威胁。这需要满足以下条件：
...
一种常见的执行机制是冷酷策略：合作直到有人背叛，然后永远背叛下去。当满足以下条件时，该策略可以维持合作：

$$
\frac{1}{1-\delta} \cdot \pi_{\text{coop}} \geq \pi_{\text{deviate}} + \frac{\delta}{1-\delta} \cdot \pi_{\text{punish}}
$$

左边是永久合作所带来的收益，而右边则是偏离行为所带来的收益，加上随后永久惩罚所带来的收益。
...
3. **序贯博弈**：求解一个三阶段进入博弈中的子博弈完美均衡，在该博弈中，企业依次决定是否进入某个市场。

4. **混合策略**：验证你为练习 1 计算出的混合策略均衡，确认该均衡使得两个参与者在其纯策略之间无差异。