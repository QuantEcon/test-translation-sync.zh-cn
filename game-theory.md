...
translation:
  title: 博弈论基础
  headings:
    Introduction to Strategic Thinking: 策略思维简介
...
博弈论为分析多个决策者进行策略互动的情形提供了一个框架。每个参与者的最优选择都取决于其他人的选择，从而产生复杂的相互依赖关系。

在经济市场中，企业在价格或产量上进行的竞争就是一种策略博弈。理解这些互动有助于预测市场结果并设计更好的机制。

## 囚徒困境

经典的囚徒困境说明了为什么理性的个体即使在合作对所有人都有利的情况下也可能不合作。两名嫌疑人被分别审讯，必须决定是坦白还是保持沉默。
...
纳什均衡是这样一种策略组合：在该组合下，任何参与者都无法通过单方面改变策略来提高自己的收益。它代表了一种稳定的结果，即在给定其他参与者策略的情况下，每个参与者的策略都是最优的。

用数学语言来说，对于策略集为 $S_i$、收益函数为 $u_i$ 的参与者 $i = 1, \ldots, n$，如果策略组合 $(s_1^*, \ldots, s_n^*)$ 满足以下条件，则称其为纳什均衡：
...
当不存在纯策略纳什均衡时，参与者可能会在不同策略之间随机选择。这些混合策略均衡需要满足无差异条件，即参与者愿意进行随机化选择。
...
在序贯博弈中，参与者按特定顺序行动，并能观察到之前的行动。这类博弈通常使用逆向归纳法进行分析，即从博弈树的末端向开端逐步推演。

其中的关键洞见是子博弈完美性：在每个决策节点上，参与者都在给定未来博弈进程的情况下做出最优选择。这消除了理性参与者不会执行的不可信威胁。

### 斯塔克尔伯格竞争

一个经典的应用是斯塔克尔伯格竞争，即一家企业（领导者）先选择产量，另一家企业（跟随者）在观察到该选择后再做出决策。

领导者会预见跟随者的反应函数，并据此做出最优选择：
...
当博弈被反复进行时，通过声誉和惩罚机制会产生新的均衡。无名氏定理表明，当参与者足够有耐心时，许多结果都可以作为均衡被维持。

其中的关键机制是：如果任何人偏离了合作路径，就会受到重新回到非合作博弈的威胁。这需要满足以下条件：
...
一种常见的执行机制是冷酷策略：合作，直到有人背叛，之后便永远背叛。当满足以下条件时，这种策略能够维持合作：

$$
\frac{1}{1-\delta} \cdot \pi_{\text{coop}} \geq \pi_{\text{deviate}} + \frac{\delta}{1-\delta} \cdot \pi_{\text{punish}}
$$

左边是永久合作所带来的收益，而右边则是偏离行为所带来的收益，加上此后永久受到惩罚所带来的收益。

## 练习

1. **计算均衡**：求出以下博弈的所有纳什均衡（纯策略与混合策略）：
...
3. **序贯博弈**：求解一个三阶段进入博弈的子博弈完美均衡，在该博弈中，企业依次决定是否进入某个市场。

4. **混合策略**：验证你在练习 1 中计算出的混合策略均衡确实使两个参与者在其纯策略之间感到无差异。