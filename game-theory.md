...
博弈论提供了一个用于分析多个决策者进行策略互动的情景的框架。每个参与者的最优选择都取决于其他参与者的选择，从而产生复杂的相互依存关系。

在经济市场中，围绕价格或数量展开竞争的企业都在进行策略博弈。理解这些互动有助于预测市场结果并设计更好的机制。

## 囚徒困境

经典的囚徒困境说明了为什么理性的个体即使在合作对所有人都有利的情况下，也可能不去合作。两名嫌疑人被分别审讯，必须决定是坦白还是保持沉默。
...
理解这个博弈有助于解释市场失灵，以及为何需要监管或合作机制。

## 纳什均衡

纳什均衡是一种策略组合，在此组合下，没有任何参与者能够通过单方面改变策略来提高自己的收益。它代表了一种稳定的结果，在给定其他参与者策略的情况下，每个参与者的策略都是最优的。

用数学语言来说，对于参与者 $i = 1, \ldots, n$，其策略集为 $S_i$，收益函数为 $u_i$，如果策略组合 $(s_1^*, \ldots, s_n^*)$ 满足以下条件，则称其为纳什均衡：
...
当不存在纯策略纳什均衡时，参与者可能会在不同策略之间进行随机化。这类混合策略均衡需要满足无差异条件，即参与者愿意进行随机化选择。
...
在序贯博弈中，参与者按特定顺序行动，并观察到之前的行动。这类博弈使用逆向归纳法进行分析，即从博弈树的末端向开端逐步推演。

其中的关键在于子博弈完美性：在每个决策节点上，参与者都会在给定未来行动的情况下做出最优选择。这消除了理性参与者不会执行的不可置信威胁。
...
其中的关键机制是：如果任何人偏离合作路径，就会受到回归非合作行为的威胁。这需要满足以下条件：
...
一种常见的执行机制是冷酷策略：合作，直到有人背叛，此后则永远背叛。当满足以下条件时，这一策略可以维持合作：

$$
\frac{1}{1-\delta} \cdot \pi_{\text{coop}} \geq \pi_{\text{deviate}} + \frac{\delta}{1-\delta} \cdot \pi_{\text{punish}}
$$

左侧是永久合作所带来的收益，而右侧则是偏离之后永久受到惩罚所带来的收益。

## 练习

1. **计算均衡**：请求出以下博弈的所有纳什均衡（纯策略和混合策略）：
...
4. **混合策略**：验证你在练习1中计算出的混合策略均衡，能使两个参与者在其各自的纯策略之间保持无差异。