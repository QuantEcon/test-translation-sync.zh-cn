...
translation:
  ...
  headings:
    ...
    The Prisoner's Dilemma::Real-World Applications: 实际应用
...
博弈论提供了一个分析多个决策者进行策略性互动情境的框架。每个参与者的最优选择都取决于其他人的选择，从而形成复杂的相互依存关系。

在经济市场中，在价格或数量上竞争的企业会参与策略博弈。理解这些互动有助于预测市场结果并设计更好的机制。

## 囚徒困境

经典的囚徒困境说明了为什么理性个体即使在合作对所有人都有利的情况下也可能不会合作。两名嫌疑人被分开审讯，必须决定是坦白还是保持沉默。
...
每个参与者都有一个占优策略——背叛，这导致了对双方都不理想的结果。
...
理解这一博弈有助于解释市场失灵以及对监管或合作机制的需求。

## 纳什均衡

纳什均衡是这样一种策略组合：在该组合下，没有任何参与者能够通过单方面改变策略来提高自己的支付。它代表了一种稳定的结果，在给定其他参与者策略的情况下，每个参与者的策略都是最优的。

用数学语言表示，对于参与者 $i = 1, \ldots, n$，其策略集为 $S_i$，支付函数为 $u_i$，如果策略组合 $(s_1^*, \ldots, s_n^*)$ 满足以下条件，则称其为纳什均衡：
...
当不存在纯策略纳什均衡时，参与者可能会在策略之间进行随机化。这些混合策略均衡需要满足无差异条件，即参与者愿意进行随机化选择。

混合策略均衡的概率可以通过求解以下方程组来计算：
...
在序贯博弈中，参与者按特定顺序行动，并能观察到之前的行动。这类博弈使用逆向归纳法进行分析，即从博弈树的末端向前推导。

其关键见解是子博弈完美性：在每个决策节点上，参与者都会根据未来的博弈进程做出最优选择。这消除了理性参与者不会执行的不可信威胁。

### 斯塔克尔伯格竞争

一个经典的应用是斯塔克尔伯格竞争，即一家企业（领导者）首先选择数量，另一家企业（跟随者）在观察到这一选择后再做决策。
...
当博弈被反复进行时，通过声誉机制和惩罚机制会产生新的均衡。无名氏定理表明，当参与者足够有耐心时，许多结果都可以作为均衡被维持。

其关键机制是威胁：如果有人偏离合作路径，就恢复到非合作博弈。这需要满足以下条件：

1. 参与者重视未来的支付（贴现因子 $\delta < 1$）
2. 偏离行为是可观察的
3. 惩罚威胁是可信的
...
左边是永久合作所带来的支付，而右边是偏离行为随后遭受永久惩罚所带来的收益。

## 练习

1. **求解均衡**：找出以下博弈的所有纳什均衡（纯策略和混合策略）：
...
3. **序贯博弈**：求解一个三阶段进入博弈的子博弈完美均衡，其中各企业依次决定是否进入某一市场。

4. **混合策略**：验证你在练习 1 中计算出的混合策略均衡，是否使双方参与者对其纯策略保持无差异。