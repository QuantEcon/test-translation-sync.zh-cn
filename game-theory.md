...
博弈论为分析多个决策者策略性互动的情境提供了一个框架。每个参与者的最优选择取决于其他人的选择,从而产生复杂的相互依赖关系。

在经济市场中,企业在价格或数量上的竞争构成策略性博弈。理解这些互动有助于预测市场结果和设计更好的机制。

## 囚徒困境

经典的囚徒困境说明了为什么理性个体可能不会合作,即使合作对每个人都有益。两名嫌疑人被分开审讯,必须决定是坦白还是保持沉默。
...
纳什均衡是这样的策略组合:没有参与者能够通过单方面改变策略来改善自己的收益。它代表了一个稳定的结果,其中每个参与者的策略在给定其他人策略的情况下都是最优的。

在数学上,对于参与者 $i = 1, \ldots, n$,其策略集为 $S_i$,收益函数为 $u_i$,策略组合 $(s_1^*, \ldots, s_n^*)$ 是纳什均衡当且仅当:
...
在序贯博弈中,参与者按特定顺序行动并观察之前的行动。这些博弈使用逆向归纳法分析,从博弈树的末端向开始处回溯。

关键洞察是子博弈完美性:在每个决策节点,参与者在给定未来博弈的情况下进行最优选择。这消除了理性参与者不会执行的不可信威胁。

### 斯塔克伯格竞争

一个经典应用是斯塔克伯格竞争,其中一家企业(领导者)首先选择产量,另一家企业(追随者)在观察到这一选择后再做决定。
...
关键机制是威胁:如果有人偏离合作路径,就回到非合作博弈。这需要:
...
2. **重复博弈**:使用冷酷触发策略计算在无限重复囚徒困境中维持合作所需的最小贴现因子。

3. **序贯博弈**:求解一个三阶段进入博弈的子博弈完美均衡,其中企业依次决定是否进入市场。

4. **混合策略**:验证你为练习1计算的混合策略均衡使两个参与者在其纯策略之间无差异。