...
博弈论提供了一个分析多个决策者进行策略互动情境的框架。每个参与者的最优选择都取决于其他参与者的选择，从而产生复杂的相互依赖关系。

在经济市场中，就价格或数量展开竞争的企业进行的就是策略博弈。理解这些互动有助于预测市场结果，并设计更好的机制。

## 囚徒困境

经典的囚徒困境说明了为什么即使合作对所有人都有利，理性的个体也可能不会选择合作。两名嫌疑犯被分开审讯，必须决定是坦白还是保持沉默。
...
纳什均衡是一种策略组合，在此组合下没有参与者能够通过单方面改变策略来提高自己的收益。它代表了一种稳定的结果，即在给定其他参与者策略的情况下，每个参与者的策略都是最优的。

从数学上讲，对于策略集为 $S_i$、收益函数为 $u_i$ 的参与者 $i = 1, \ldots, n$，如果满足以下条件，策略组合 $(s_1^*, \ldots, s_n^*)$ 就是纳什均衡：
...
当不存在纯策略纳什均衡时，参与者可能会对策略进行随机化。这些混合策略均衡需要满足无差异条件，即参与者愿意进行随机化选择。
...
在序贯博弈中，参与者按照特定的顺序行动，并能观察到之前的行动。这类博弈使用逆向归纳法进行分析，即从博弈树的末端向开端逐步推导。

其中的关键洞见是子博弈完美性：在每个决策节点上，参与者都会在考虑未来行动的情况下做出最优选择。这消除了理性参与者不会执行的不可信威胁。

### 斯塔克尔伯格竞争

一个经典的应用是斯塔克尔伯格竞争，其中一家企业（领导者）首先选择数量，另一家企业（跟随者）在做出决策之前观察到这一选择。

领导者预判跟随者的反应函数，并做出最优选择：
...
当博弈被反复进行时，通过声誉和惩罚机制会产生新的均衡。无名氏定理表明，当参与者足够有耐心时，许多结果都可以作为均衡被维持下去。

其中的关键机制是：如果有人偏离合作路径，就会威胁回归非合作行为。这需要满足以下条件：
...
左边是持续合作所带来的收益，而右边则是偏离行为之后紧接着永久惩罚所带来的收益。
...
2. **重复博弈**：计算在无限重复的囚徒困境中，使用冷酷策略维持合作所需的最低贴现因子。

3. **序贯博弈**：求解一个三阶段进入博弈的子博弈完美均衡，其中企业依次决定是否进入某个市场。

4. **混合策略**：验证你在练习 1 中计算得到的混合策略均衡是否使两个参与者在其纯策略之间保持无差异。