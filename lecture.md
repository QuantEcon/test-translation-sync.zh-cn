...
kernelspec:
  ...
  name: python3
heading-map:
  Programming for Economics: 经济学编程
  ...
  '"Edge Cases" and [Special] {Characters}': '"边缘案例"和[特殊]{字符}'
  '"Edge Cases" and [Special] {Characters}::Using `matplotlib`''s `plt.subplot()` for Multiple Plots': 使用 `matplotlib` 的 `plt.subplot()` 创建多个图形
  Summary: 总结
---
...
不同的数组创建方法有不同的用途。`np.arange()` 函数创建具有指定步长的数组：
...
**pandas** 库提供了强大的数据操作功能。DataFrames 是二维标记数据结构。
...
*Matplotlib* 是 Python 的基础绘图库。它提供对图形外观的精细控制。

### `plt.plot()` 函数

基本折线图通过 `plt.plot(x, y)` 创建：
...
关于 Python 在经济学中应用的常见问题：

- **问**：`numpy` 和纯 Python 哪个更快？
- **答**：对于数值运算，`numpy` 通常快 10-100 倍

- **问**：我可以在大型数据集上使用 `pandas` 吗？
...
### 使用 `matplotlib` 的 `plt.subplot()` 创建多个图形

subplot 函数在一个图形中创建多个图：
...
像 **numpy**、*pandas* 和 `matplotlib` 这样的编程工具对于现代经济研究至关重要。掌握这些库可以高效地分析数据和可视化结果。
