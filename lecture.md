...
translation:
  ...
  headings:
    ...
    'Special Cases: $\LaTeX$ in Headings': 标题中的特殊情况：$\LaTeX$
    'Special Cases: $\LaTeX$ in Headings::The $\beta$-Coefficient in Regression': 回归分析中的 $\beta$ 系数
...
本讲座涵盖了对现代经济分析至关重要的编程概念和工具。

## 使用 `numpy` 数组

`numpy` 库为数值计算提供了高效的数组运算。数组是 Python 科学计算的基础。
...
不同的数组创建方法用于不同的目的。`np.arange()` 函数创建具有指定步长的数组：

```python
x = np.arange(0, 10, 0.5)  # From 0 to 10, step 0.5
```

而 `np.linspace()` 则创建具有指定点数的数组：
...
[QuantEcon](https://quantecon.org) 项目提供了用于经济建模的专业工具。请访问 [文档](https://quanteconpy.readthedocs.io/) 了解详情。
...
关于经济学中 Python 使用的常见问题：

- **问**：`numpy` 和普通 Python 相比哪个更快？
- **答**：对于数值运算，`numpy` 通常快 10-100 倍

- **问**：我可以用 `pandas` 处理大型数据集吗？
- **答**：可以，但对于超过内存大小的数据集，建议考虑使用 `dask`
...
像 **numpy**、*pandas* 和 `matplotlib` 这样的编程工具对于现代经济学研究至关重要。掌握这些库可以高效地分析数据和可视化结果。
