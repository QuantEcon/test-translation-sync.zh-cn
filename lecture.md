...
NumPy 数组支持向量化操作，从而避免了缓慢的 Python 循环：
...
不同的数组创建方法用于不同的目的。`np.arange()` 函数可以创建具有指定步长的数组：

```python
x = np.arange(0, 10, 0.5)  # From 0 to 10, step 0.5
```

同时，`np.linspace()` 可以创建具有指定点数的数组：
...
**pandas** 库提供了强大的数据处理能力。DataFrames 是二维带标签的数据结构。
...
[QuantEcon](https://quantecon.org) 项目提供了用于经济建模的专业工具。访问 [文档](https://quanteconpy.readthedocs.io/) 了解详情。
