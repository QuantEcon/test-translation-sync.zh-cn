...
translation:
  ...
  headings:
    ...
    'Working with **Pandas DataFrames**::Reading Data: `pd.read_csv()` vs `pd.read_excel()`': 读取数据：`pd.read_csv()` 与 `pd.read_excel()` 的对比
    ...
    '"Edge Cases" and [Special] {Characters}': “边缘情况”与 [特殊] {字符}
    '"Edge Cases" and [Special] {Characters}::Using `matplotlib`''s `plt.subplot()` for Multiple Plots': 使用 `matplotlib` 的 `plt.subplot()` 绘制多个图形
    Summary: 摘要
---

# 经济学编程

本讲座涵盖了现代经济分析中至关重要的编程概念和工具。

## 使用 `numpy` 数组

`numpy` 库为数值计算提供了高效的数组运算功能。数组是 Python 科学计算的基础。

NumPy 数组支持向量化运算，从而避免了缓慢的 Python 循环：
...
不同的数组创建方法适用于不同的用途。`np.arange()` 函数使用指定的步长创建数组：

```python
x = np.arange(0, 10, 0.5)  # From 0 to 10, step 0.5
```

与此同时，`np.linspace()` 使用指定的点数创建数组：
...
**pandas** 库提供了强大的数据处理能力。DataFrame 是二维带标签的数据结构。

### 读取数据：`pd.read_csv()` 与 `pd.read_excel()` 的对比
...
*Matplotlib* 是 Python 的基础绘图库，它提供了对绘图外观的精细控制。

### `plt.plot()` 函数

可以使用 `plt.plot(x, y)` 创建基本折线图：
...
[QuantEcon](https://quantecon.org) 项目提供了专门用于经济建模的工具。请访问[文档](https://quanteconpy.readthedocs.io/)以了解详情。
...
关于 Python 用于经济学的常见问题：

- **问**：`numpy` 和普通 Python 哪个更快？
- **答**：对于数值运算，`numpy` 通常快 10 到 100 倍

- **问**：我可以使用 `pandas` 处理大型数据集吗？
- **答**：可以，但对于超过内存容量的数据集，建议考虑使用 `dask`

## “边缘情况”与 [特殊] {字符}

本节测试标题中的各种特殊字符。

### 使用 `matplotlib` 的 `plt.subplot()` 绘制多个图形

subplot 函数可在一个图形中创建多个绘图：
...
**numpy**、*pandas* 和 `matplotlib` 等编程工具对现代经济研究至关重要。掌握这些库可以帮助你高效地分析数据和可视化结果。
