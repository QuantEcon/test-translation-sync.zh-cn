...
translation:
  ...
  headings:
    ...
    Using [QuantEcon](https://quantecon.org) Libraries::Installing `quantecon` Package: 安装 `quantecon` 软件包
...
NumPy 数组支持向量化运算，避免了缓慢的 Python 循环：
...
不同的数组创建方法用于不同的目的。`np.arange()` 函数用于创建具有指定步长的数组：

```python
x = np.arange(0, 10, 0.5)  # From 0 to 10, step 0.5
```

与此同时，`np.linspace()` 用于创建具有指定点数的数组：
...
**pandas** 库提供了强大的数据处理能力。DataFrame 是二维标记数据结构。
...
[QuantEcon](https://quantecon.org) 项目为经济建模提供了专业的工具。请访问 [documentation](https://quanteconpy.readthedocs.io/) 了解详情。
...
- **问**：我可以将 `pandas` 用于大型数据集吗？
- **答**：可以，但对于超出内存容量的数据集，请考虑使用 `dask`
...
像 **numpy**、*pandas* 和 `matplotlib` 这样的编程工具对现代经济研究至关重要。掌握这些库，可以高效地分析数据并将结果可视化。
